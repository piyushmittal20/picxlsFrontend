			...
			<div className="d-flex flex-column flex-lg-row flex-column-fluid" id="kt_login">
				<div className="d-flex flex-column flex-lg-row-auto bg-primary w-lg-600px pt-15 pt-lg-0">
					<div className="d-flex flex-column-auto flex-column pt-lg-40 pt-15 text-center">
						<img className="logo-width" src="../assets/media/picxls-logo.png"/>
						<br/> 
					...
					<div className="d-flex flex-column-fluid flex-center py-10">
						<form onSubmit={handleSubmit} className="form w-100" novalidate="novalidate" >
							<div class="pb-5 pb-lg-15">
								<h3 class="fw-bolder text-dark display-6">Welcome to Picxls</h3>
							</div>
							<div className="fv-row mb-10">
								<label className="form-label fs-6 fw-bolder text-dark">Email</label>
								<input className="form-control form-control-lg form-control-solid" onChange={(e) => setEmail(e.target.value)} type="text" name="username" autocomplete="off" value={email} />
								<span className="error-msg">{emailErr}</span>
							</div>
							<div className="fv-row mb-10">
								<label className="form-label fs-6 fw-bolder text-dark pt-5">Password</label>
								<input className="form-control form-control-lg form-control-solid" onChange={(e) => setPassword(e.target.value)} type="password" name="password" autocomplete="off" value={password} />
								<span className="error-msg">{passwordErr}</span>
							</div>
							<div className="pb-lg-0 pb-5">
								{loading ? 
									<button type="submit" disabled="true" className="btn btn-primary fw-bolder fs-6 px-8 py-4 my-3 me-3 login-btn">
										<p>Processing...</p> <Spinner style={{marginLeft: '10px'}} animation="border" />
									</button>
									: 
									<button type="submit" className="btn btn-primary fw-bolder fs-6 px-8 py-4 my-3 me-3 login-btn">Sign In</button>
								}
							</div>
						</form>
					</div>
